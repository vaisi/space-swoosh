...
// EncounterCatalog.js is the source of truth for gauntlets; OpenSpaceWeather.js
// for KM weather, belt density, and storms. Full-sky weather omits wormhole
// pair/focus (portals are helpers). Storm quiet / chain / gap-cap emit with
// the weather table. Both emit into game-constants.json
// and GeneratedJourneyData.
...
import {
  OPEN_SPACE_BELT,
  OPEN_SPACE_DUAL_STORM_FROM_KM,
  OPEN_SPACE_FULL_ROSTER_KM,
  OPEN_SPACE_FULL_SKY,
  OPEN_SPACE_PAIRED_FROM_KM,
  OPEN_SPACE_STORM_DENSE_FROM_KM,
  ...
  OPEN_SPACE_WEATHER,
} from '../src/config/OpenSpaceWeather.js';
...
const out = {
  version: 3,
  source: 'GameConfig + RunProfile + JourneyConfig + JourneyNarrative + Logbook + CopyBank + EncounterCatalog + OpenSpaceWeather',
  simDt: 1 / 60,
  ...
  openSpaceWeather: {
    pairedFromKm: OPEN_SPACE_PAIRED_FROM_KM,
    stormRepeatKm: OPEN_SPACE_STORM_REPEAT_KM,
    stormDenseRepeatKm: OPEN_SPACE_STORM_DENSE_REPEAT_KM,
    stormDenseFromKm: OPEN_SPACE_STORM_DENSE_FROM_KM,
    dualStormFromKm: OPEN_SPACE_DUAL_STORM_FROM_KM,
    fullRosterKm: OPEN_SPACE_FULL_ROSTER_KM,
    stormRepeatCount: OPEN_SPACE_STORM_REPEAT_COUNT,
...
const weatherBandLines = OPEN_SPACE_WEATHER.map((band) => `        OpenSpaceWeatherBand(
            fromKm: ${band.fromKm},
            pair: ${optSwiftStr(band.pair)},
            combo: ${optSwiftStr(band.combo)},
            focus: ${optSwiftStr(band.focus)}
        )`).join(',\n');
const fullSkyLines = OPEN_SPACE_FULL_SKY.map((band) => `        OpenSpaceSkyShift(
            pair: ${optSwiftStr(band.pair)},
            combo: ${optSwiftStr(band.combo)},
            focus: ${optSwiftStr(band.focus)}
        )`).join(',\n');
const beltLines = OPEN_SPACE_BELT.map((band) => `        OpenSpaceBeltBand(
            fromKm: ${band.fromKm},
            minGapFrac: ${band.minGapFrac},
            gapSpread: ${band.gapSpread},
            simpleChance: ${band.simpleChance},
...
${encounterLines}
    ...
    static let openSpacePairedFromKm: CGFloat = ${OPEN_SPACE_PAIRED_FROM_KM}
    static let openSpaceStormRepeatKm: CGFloat = ${OPEN_SPACE_STORM_REPEAT_KM}
    static let openSpaceStormDenseRepeatKm: CGFloat = ${OPEN_SPACE_STORM_DENSE_REPEAT_KM}
    static let openSpaceStormDenseFromKm: CGFloat = ${OPEN_SPACE_STORM_DENSE_FROM_KM}
    static let openSpaceDualStormFromKm: CGFloat = ${OPEN_SPACE_DUAL_STORM_FROM_KM}
    static let openSpaceFullRosterKm: CGFloat = ${OPEN_SPACE_FULL_ROSTER_KM}
    static let openSpaceStormRepeatCount = ${OPEN_SPACE_STORM_REPEAT_COUNT}
